...
public final class UserMenu extends javax.swing.JFrame {
    ...
    if(filtroBusqueda.isEmpty()){
        String queryConsulta = "SELECT nombreEmp, apellidos, tipoDocumento, documento, correo, nombreSucursal, nombrePuestoTrabajo FROM empleado INNER JOIN Sucursal ON idSucursal = FK_idSucursal INNER JOIN PuestoTrabajo ON(FK_idPuestoTrabajo = idPuestoTrabajo);";
            try{
                ...
                st = connection.createStatement(); //Creamos la consulta query a la base de datos 
                ...
                while(rs.next()){
                    ...
                    tblEmpleados.setModel(contenidoTablaEmpleados);
                         System.out.println("nombre: "+empleado[0]+", apellido: "+empleado[1]+", documento: "+empleado[2]+" "
                            +empleado[3]+" ,correo"+empleado[4]+", Sucursal: "+empleado[5]+", Puesto Trabajo: "+empleado[6]);
                }
        ...
        String query = "SELECT nombreEmp, apellidos, tipoDocumento, documento, correo, nombreSucursal, nombrePuestoTrabajo FROM empleado INNER JOIN Sucursal ON (idSucursal = FK_idSucursal) INNER JOIN PuestoTrabajo ON(FK_idPuestoTrabajo = idPuestoTrabajo)WHERE nombreEmp LIKE '%" + filtroBusqueda + "%' OR apellidos LIKE '%" + filtroBusqueda + "%';";
        try{
            connection = conexion.getConnection();
    ...
    private void initComponents() {
        ...
        btnPuestosTrabajo.setIcon(new javax.swing.ImageIcon(getClass().getResource("/assets/AddEmpleado.png"))); // NOI18N
    ...
    private void tblEmpleadosMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_tblEmpleadosMouseClicked
        ...
        if(row < 0){
            ...
            String puestoTrabajo = (String)tblEmpleados.getValueAt(row, 6);
            ...
            String queryidEmpleado ="SELECT idEmp FROM empleado WHERE documento = '"+document+"';";
                try{
                   ...
                   while(rs.next()){
                       ...
                       showUserForm.receiveData(idEmp, name, lastname, document_type, document, email, sucursal, puestoTrabajo);
